...
function Core( config ){
	...
	this.interpertInstruction = function( config, instruction ){
	...
	this.step = function(){
		...
		post_processors.forEach( function( processor ){
			processor.apply( core, [ config, state ] );
		} );
	};
	
	this.printState = Core.printState;
	
	LOGGER.info( 'Systematic Core Ready.' );
}

Core.printState = function( config, state ){
	...
	t.push([ 'Instruction Buffer', '0b' + instruction.toBinaryString( config.instruction_length ), this.interpertInstruction( config, instruction ).toString() ]);
