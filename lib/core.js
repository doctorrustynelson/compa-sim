...
function Core( config ){
	...
	this.step = function(){
		...
		postprocessors.forEach( function( processor ){
			processor.apply( this, config, state );
		} );
	};
...
Core.prototype.printState = function( config, state ){
	...
	t.push([ 'Instruction Buffer', '0b' + instruction.toBinaryString( config.instruction_length ), this.interpertInstruction( instruction ).toString() ]);
